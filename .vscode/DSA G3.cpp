...
void quickSortArr(vector<int>& arr, int start, int end) {
    if(start < end) {
        int pivotIndex = partitionArr(arr, start, end);DSA
...
int main() {
    int n, searchKey;
    cout << "How many elements do you want to generate? (max 1000): ";
    cin >> n;

    if(n > 1000) {
        cout << "\nOops! Maximum allowed input is 1000 elements. Please try again.\n";
        return 1;
    }
    ...
    cout << "\nQuick Sort Run_Time: " << timeQuick.count() << " microseconds\n";

    cout << "\nMerge Sort:\n";
    for(int num : mergeSorted) cout << num << " ";
    cout << "\nMerge Sort Run_Time: " << timeMerge.count() << " microseconds\n";
    ...
    cout << " | Time: " << timeBinary.count() << " microseconds";

    cout << "\nExponential Search Result: ";
    if(exponentialResult != -1) cout << "Found";
    else cout << "Not Found";
    cout << " | Time: " << timeExponential.count() << " microseconds\n";
